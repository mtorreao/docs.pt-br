...
|**-e**[**xecution**] {**on** &#124; **off**}|Ativa ou desativa o mecanismo que verifica a permissão para execução antes do código iniciar a execução. **Observação:**  Essa opção é removida no .NET Framework 4 e em versões posteriores. Para saber mais, confira [Alterações de segurança](/previous-versions/dotnet/framework/security/security-changes).|  
