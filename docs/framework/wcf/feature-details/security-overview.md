...
### <a name="standards-and-interoperability"></a>Padrões e interoperabilidade  
 ...
 O WCF dá suporte a uma ampla variedade de cenários de interoperabilidade. A <xref:System.ServiceModel.BasicHttpBinding> classe é direcionada ao BSP (perfil de segurança básica) e a <xref:System.ServiceModel.WSHttpBinding> classe é direcionada aos padrões de segurança mais recentes, como WS-Security 1,1 e WS-SecureConversation. Ao aderir a esses padrões, a segurança do WCF pode interoperar e integrar os serviços da Web que são hospedados em sistemas operacionais e plataformas diferentes do Microsoft Windows.  
