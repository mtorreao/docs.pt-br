...
- Concedendo permissões na chave privada do certificado  
     ...
     As linhas a seguir no arquivo de Setup.bat lote tornam o certificado do servidor armazenado no repositório LocalMachine acessível para a conta de processo de trabalho do ASP.NET.  
    ...
    > [!NOTE]
    > Se você estiver usando um que não seja U. S. Edição em inglês do Windows você deve editar o arquivo de Setup.bat e substituir o `NT AUTHORITY\NETWORK SERVICE` nome da conta pelo equivalente regional.  
...
1. Verifique se o caminho inclui a pasta onde Makecert.exe e FindPrivateKey.exe estão localizados.  
  
2. Execute Setup.bat na pasta de instalação de exemplo em um Prompt de Comando do Desenvolvedor para Visual Studio aberto com privilégios de administrador. Isso instala todos os certificados necessários para executar o exemplo.  
...
2. Copie os arquivos de programa do serviço do \inetpub\wwwroot\servicemodelsamples para o diretório virtual no computador do serviço. Certifique-se de copiar os arquivos no subdiretório \bin. Copie também os arquivos de Setup.bat e Cleanup.bat para o computador de serviço.  
...
6. Edite Web.config para refletir o novo nome do certificado (no atributo FindValue no elemento Service-Certificate), que é o mesmo que o nome de domínio totalmente qualificado do computador`.`  
