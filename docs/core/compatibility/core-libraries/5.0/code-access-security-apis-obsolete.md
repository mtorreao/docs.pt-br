...
A [CAS (segurança de acesso ao código)](../../../../framework/misc/code-access-security.md) é uma tecnologia herdada sem suporte. A infraestrutura para habilitar o CAS existe somente no .NET Framework 2. x-4. x, mas foi preterida e não está recebendo correções de serviço ou de segurança.

Devido à reprovação do CAS, a [infra-estrutura de suporte não foi levada para o .NET Core](../../../porting/net-framework-tech-unavailable.md) ou o .NET 5.0 +. No entanto, as APIs foram encaminhadas para que os aplicativos pudessem compilar entre .NET Framework e o .NET Core. Isso levou a cenários de "falha aberta", onde existem algumas APIs relacionadas a CAS e que podem ser chamadas, mas não executam nenhuma ação em tempo de execução. Isso pode levar a problemas de segurança para os componentes que esperam que o tempo de execução obedeça atributos relacionados ao CAS ou chamadas de API programática. Para se comunicar melhor de que o tempo de execução não respeita esses atributos ou APIs, tornamos obsoleto a maioria deles no .NET 5,0.
...
- Se você estiver exigindo <xref:System.Security.Permissions.PrincipalPermission> , consulte as diretrizes para [PrincipalPermissionAttribute é obsoleto como erro](principalpermissionattribute-obsolete.md). Essa orientação se aplica para o <xref:System.Security.Permissions.PrincipalPermission> e o <xref:System.Security.Permissions.PrincipalPermissionAttribute> .
